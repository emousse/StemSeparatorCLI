...
class TestLarsServiceRealAudio:
    ...
    @pytest.mark.skipif(
        not is_lars_service_available(), reason="LARS service binary not available"
    )
        ...
        for stem_name, stem_path in [
            ...
            try:
                ...
                assert data.ndim in [
                    1,
                    2,
                ], f"{stem_name} has invalid shape: {data.shape}"
    ...
    def test_lars_service_json_parseable_after_separation(
        ...
        cmd = [
            ...
            "separate",
            "--input",
            str(real_audio_file.absolute()),
            "--output-dir",
            str(output_dir.absolute()),
            "--stems",
            ",".join(SUPPORTED_STEMS),
            "--device",
            "auto",
            "--format",
            "wav",
        ...
        has_progress = (
            "loading" in stderr_lower
            or "separate" in stderr_lower
            or "%" in stderr  # Progress bars
            or "it/s" in stderr  # tqdm format
        )
        assert (
            has_progress
