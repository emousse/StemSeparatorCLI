...
def test_queue_widget_update_status(qapp, reset_singletons, mock_audio_file):
    ...
    assert "1 tasks" in widget.status_label.text() or "1 task" in widget.status_label.text()


@pytest.mark.unit
@patch('ui.widgets.queue_widget.QueueWorker')
def test_queue_widget_start_processing(mock_worker_class, qapp, reset_singletons, mock_audio_file):
