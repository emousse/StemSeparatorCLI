...
from core.blackhole_installer import BlackHoleInstaller, BlackHoleStatus, get_blackhole_installer
...
class TestBlackHoleInstaller:
    ...
    def test_get_status_installed_with_device(self, mock_system, mock_run):
        ...
        mock_run.side_effect = [
            ...
            Mock(returncode=0),  # brew --version (in check_blackhole_installed -> check_homebrew)
            Mock(returncode=0, stdout="blackhole-2ch 0.4.0\n")  # brew list
        ]
    ...
    def test_get_status_installed_no_device(self, mock_system, mock_run):
        ...
        with patch.object(BlackHoleInstaller, 'check_blackhole_device', return_value=False):
            installer = BlackHoleInstaller()
            status = installer.get_status()
    ...
    def test_install_blackhole_already_installed(self, mock_system, mock_run):
        ...
        mock_system.return_value = 'Darwin'
        mock_run.side_effect = [
            Mock(returncode=0),  # brew --version (in install_blackhole -> check_homebrew)
            Mock(returncode=0),  # brew --version (in check_blackhole_installed -> check_homebrew)
            Mock(returncode=0, stdout="blackhole-2ch 0.4.0\n")  # brew list
        ]
    ...
    def test_install_blackhole_with_callback(self, mock_system, mock_run):
        ...
        installer = BlackHoleInstaller()
        success, error = installer.install_blackhole(progress_callback=progress_callback)
