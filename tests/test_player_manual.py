...
def test_volume_controls():
    ...
    try:
        ...
        assert player.stem_settings['vocals'].volume == 1.0, "Volume should be clamped to 1.0"

        player.set_stem_volume('vocals', -0.5)
        assert player.stem_settings['vocals'].volume == 0.0, "Volume should be clamped to 0.0"
...
def test_mixing():
    ...
    try:
        ...
        assert mixed.shape == (2, 44100), f"Mixed shape should be (2, 44100), got {mixed.shape}"
        assert mixed.dtype == np.float32, f"Mixed dtype should be float32, got {mixed.dtype}"
...
def test_export():
    ...
    try:
        ...
        assert sr == player.sample_rate, f"Sample rate mismatch: {sr} vs {player.sample_rate}"
